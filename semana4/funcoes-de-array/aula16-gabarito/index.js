...
function imprimirExtrato(){
    ...
    arrDespesas.forEach((despesa) => {
        ...
        if(despesa.tipo === "viagem"){gastoViagem += despesa.valor}
        else if(despesa.tipo === "utilidades"){gastoUtilidades += despesa.valor}
        else if (despesa.tipo === "alimentação"){gastoAlimentacao += despesa.valor}
        })
...
function filtrarDespesas(){
    ...
    let despesasFiltradas = arrDespesas.filter ((despesa) => {
        if(despesa.valor <= valorMax && despesa.valor >= valorMin && (despesa.tipo === tipoFiltro || tipoFiltro === "todos")){
            return true
        }
